...
#x: (*, s)
#y: (*, s)
def central_diff_second_1d(x, h, fix_x_bnd=False):
    ddx = (torch.roll(x, -1, dims=-1) - 2*torch.roll(x, 0, dims=-1) + torch.roll(x, 1, dims=-1))/(h**2)

    if fix_x_bnd:
        ddx[...,0] = (x[...,2] - 2*x[...,1] + x[...,0])/(2*h**2)
        ddx[...,-1] = (x[...,-1] - 2*x[...,-2] + x[...,-3])/(2*h**2)
    
    return ddx

#x: (*, s1, s2)
#y: (*, s1, s2)
def central_diff_second_2d(x, h, fix_x_bnd=False, fix_y_bnd=False):
    if isinstance(h, float):
        h = [h, h]

    ddx = (torch.roll(x, -1, dims=-2) - 2*torch.roll(x, 0, dims=-2) + torch.roll(x, 1, dims=-2))/(h[0]**2)
    ddy = (torch.roll(x, -1, dims=-1) - 2*torch.roll(x, 0, dims=-1) + torch.roll(x, 1, dims=-1))/(h[1]**2)

    if fix_x_bnd:
        ddx[...,0,:] = (x[...,2,:] -2*x[...,1,:] + x[...,0,:])/(2*h[0]**2)
        ddx[...,-1,:] = (x[...,-1,:] - 2*x[...,-2,:] + x[...,-3,:])/(2*h[0]**2)
    
    if fix_y_bnd:
        ddy[...,:,0] = (x[...,:,2] - 2*x[...,:,1] + x[...,:,0])/(2*h[1]**2)
        ddy[...,:,-1] = (x[...,:,-1] - 2*x[...,:,-2] + x[...,:,-3])/(2*h[1]**2)
        
    return ddx, ddy

#x: (*, s1, s2, s3)
#y: (*, s1, s2, s3)
def central_diff_second_3d(x, h, fix_x_bnd=False, fix_y_bnd=False, fix_z_bnd=False):
    if isinstance(h, float):
        h = [h, h, h]

    ddx = (torch.roll(x, -1, dims=-3) - 2*torch.roll(x, 0, dims=-3) + torch.roll(x, 1, dims=-3))/(h[0]**2)
    ddy = (torch.roll(x, -1, dims=-2) - 2*torch.roll(x, 0, dims=-2) + torch.roll(x, 1, dims=-2))/(h[1]**2)
    ddz = (torch.roll(x, -1, dims=-1) - 2*torch.roll(x, 1, dims=-1) + torch.roll(x, 1, dims=-1))/(h[2]**2)
    ...
    if fix_z_bnd:
        ddz[...,:,:,0] = (x[...,:,:,2] - 2*x[...,:,:,1] + x[...,:,:,0])/(2*h[2]**2)
        ddz[...,:,:,-1] = (x[...,:,:,-1] - 2*x[...,:,:,-2] + x[...,:,:,-3])/(2*h[2]**2)
...
def central_diff_second_1d(x, h, fix_x_bnd=False):
    ...
    if fix_x_bnd:
        ddx[...,0] = (x[...,2] - 2*x[...,1] )/(2*h**2)
        ddx[...,-1] = ( x[...,-3]- 2*x[...,-2] )/(2*h**2)
...
def central_diff_second_2d(x, h, fix_x_bnd=False, fix_y_bnd=False):
    ...
    if fix_x_bnd:
        ddx[...,0,:] = (x[...,2,:] -2*x[...,1,:] )/(2*h[0]**2)
        ddx[...,-1,:] = ( x[...,-3,:] - 2*x[...,-2,:])/(2*h[0]**2)
    
    if fix_y_bnd:
        ddy[...,:,0] = ( x[...,:,0] - 2*x[...,:,1] )/(2*h[1]**2)
        ddy[...,:,-1] = (x[...,:,-3] - 2*x[...,:,-2])/(2*h[1]**2)
...
def central_diff_second_3d(x, h, fix_x_bnd=False, fix_y_bnd=False, fix_z_bnd=False):
    ...
    if fix_y_bnd:
        ddy[...,:,0,:] = (x[...,:,2,:] - 2*x[...,:,1,:] )/(2*h[1]**2)
        ddy[...,:,-1,:] = (x[...,:,-3,:] - 2*x[...,:,-2,:] )/(2*h[1]**2)
    
    if fix_z_bnd:
        ddz[...,:,:,0] = (x[...,:,:,2] - 2*x[...,:,:,1] )/(2*h[2]**2)
        ddz[...,:,:,-1] = (x[...,:,:,-3] - 2*x[...,:,:,-2] )/(2*h[2]**2)
...
class H2Loss(object):
    ...
    def compute_terms(self, x, y, h):
        ...
        if self.d == 1:
            ...
            x_x2 = central_diff_second_1d(x, h[0], fix_x_bnd=self.fix_x_bnd)
            y_x2 = central_diff_second_1d(y, h[0], fix_x_bnd=self.fix_x_bnd)
            ...
            x_x2, x_y2 = central_diff_second_2d(x, h, fix_x_bnd=self.fix_x_bnd, fix_y_bnd=self.fix_y_bnd)
            y_x2, y_y2 = central_diff_second_2d(y, h, fix_x_bnd=self.fix_x_bnd, fix_y_bnd=self.fix_y_bnd)
            
            dict_x[3] = torch.flatten(x_x2, start_dim=-2)
            dict_x[4] = torch.flatten(x_y2, start_dim=-2)

            dict_y[3] = torch.flatten(y_x2, start_dim=-2)
            dict_y[4] = torch.flatten(y_y2, start_dim=-2)
            ...
            x_x2, x_y2, x_z2 = central_diff_second_3d(x, h, fix_x_bnd=self.fix_x_bnd, fix_y_bnd=self.fix_y_bnd, fix_z_bnd=self.fix_z_bnd)
            y_x2, y_y2, y_z2 = central_diff_second_3d(y, h, fix_x_bnd=self.fix_x_bnd, fix_y_bnd=self.fix_y_bnd, fix_z_bnd=self.fix_z_bnd)

            dict_x[4] = torch.flatten(x_x2, start_dim=-3)
            dict_x[5] = torch.flatten(x_y2, start_dim=-3)
            dict_x[6] = torch.flatten(x_z2, start_dim=-3)

            dict_y[4] = torch.flatten(y_x2, start_dim=-3)
            dict_y[5] = torch.flatten(y_y2, start_dim=-3)
            dict_y[6] = torch.flatten(y_z2, start_dim=-3)
